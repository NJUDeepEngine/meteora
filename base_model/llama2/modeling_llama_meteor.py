...
def lora_gate_loss_func(
    moe_logits: torch.Tensor, num_loras=10, top_k=2, attention_mask: Optional[torch.Tensor] = None, moe_labels=None
) -> float:
    r"""
...
class LlamaMeteorRMSNorm(nn.Module):
...
ALL_LAYERNORM_LAYERS.append(LlamaMeteorRMSNorm)


class LlamaMeteorRotaryEmbedding(nn.Module):
...
class LlamaMeteorLinearScalingRotaryEmbedding(LlamaMeteorRotaryEmbedding):
    """LlamaMeteorRotaryEmbedding extended with linear scaling. Credits to the Reddit user /u/kaiokendev"""
...
class LlamaMeteorDynamicNTKScalingRotaryEmbedding(LlamaMeteorRotaryEmbedding):
...
class LlamaMeteorMLP(nn.Module):
...
class LlamaMeteorAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: LlamaMeteorConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            self.rotary_emb = LlamaMeteorRotaryEmbedding(
                self.head_dim,
            ...
            if scaling_type == "linear":
                self.rotary_emb = LlamaMeteorLinearScalingRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                ...
                self.rotary_emb = LlamaMeteorDynamicNTKScalingRotaryEmbedding(
                    self.head_dim,
                    max_position_embeddings=self.max_position_embeddings,
                    scaling_factor=scaling_factor,
                    base=self.rope_theta,
...
class LlamaMeteorFlashAttention2(LlamaMeteorAttention):
    """
...
class LlamaMeteorSdpaAttention(LlamaMeteorAttention):
    """
...
LLAMA_ATTENTION_CLASSES = {
    "eager": LlamaMeteorAttention,
    "flash_attention_2": LlamaMeteorFlashAttention2,
    "sdpa": LlamaMeteorSdpaAttention,
}


class LlamaMeteorDecoderLayer(nn.Module):
    def __init__(self, config: LlamaMeteorConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = LLAMA_ATTENTION_CLASSES[config._attn_implementation](config=config, layer_idx=layer_idx)

        self.mlp = LlamaMeteorMLP(config)
        self.input_layernorm = LlamaMeteorRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = LlamaMeteorRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
LLAMA_METEOR_START_DOCSTRING = r"""
...
@add_start_docstrings(
    ...
    LLAMA_METEOR_START_DOCSTRING,
)
class LlamaMeteorPreTrainedModel(PreTrainedModel):
    config_class = LlamaMeteorConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["LlamaMeteorDecoderLayer"]
...
LLAMA_METEOR_INPUTS_DOCSTRING = r"""
    Args:
...
@add_start_docstrings(
    "The bare LLaMA Model outputting raw hidden-states without any specific head on top.",
    LLAMA_METEOR_INPUTS_DOCSTRING,
)
class LlamaMeteorModel(LlamaMeteorPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`LlamaMeteorDecoderLayer`]

    Args:
        config: LlamaMeteorConfig
    """

    def __init__(self, config: LlamaMeteorConfig):
        ...
        self.layers = nn.ModuleList(
            [LlamaMeteorDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self._use_sdpa = config._attn_implementation == "sdpa"
        self._use_flash_attention_2 = config._attn_implementation == "flash_attention_2"
        self.norm = LlamaMeteorRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    @add_start_docstrings_to_model_forward(LLAMA_METEOR_INPUTS_DOCSTRING)
    def forward(
        self,
...
class LlamaMeteorForCausalLM(LlamaMeteorPreTrainedModel):
...
class LlamaMeteorForSequenceClassification(LlamaMeteorPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
