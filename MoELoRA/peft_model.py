...
from peft.peft_model import __version__
from peft.config import PeftConfig
from MoELoRA.lora_model import LoraModel
...
from MoELoRA.peft_utils import (
    load_peft_weights,
    ...
    **Attributes**:
    ...
    def __init__(self, model: PreTrainedModel, peft_config: PeftConfig, adapter_name: str = "default") -> None:
        ...
        if self._is_prompt_learning:
            ...
            # print("in peft init,", peft_config.peft_type)
            self._peft_config = None
            cls = PEFT_TYPE_TO_MODEL_MAPPING[peft_config.peft_type]
            self.base_model = cls(model, {adapter_name: peft_config}, adapter_name)
            # print("in peft init, after cls func", model, self.base_model)
    ...
    def from_pretrained(
        ...
        **kwargs: Any,
        ...
        # print("peft config is:", config)
        if (getattr(model, "hf_device_map", None) is not None) and len(
            set(model.hf_device_map.values()).intersection({"cpu", "disk"})
        ) > 0:
            remove_hook_from_submodules(model)
        # print("config", config.is_prompt_learning, config.task_type)    
        ...
        if config.task_type not in MODEL_TYPE_TO_PEFT_MODEL_MAPPING.keys():
            ...
            # print("in CAUSAL LM, before peft mapping", model)
    ...
    def from_pretrained_multi(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            model_id (`str` or `os.PathLike`):
                The name of the PEFT configuration to use. Can be either:
                    ...
                    - A path to a directory containing a PEFT configuration file saved using the `save_pretrained`
                      method (`./my_peft_config_directory/`).
            adapter_names (`dict`, *optional*, defaults to `"default"`):
                The name of the adapter to be loaded. This is useful for loading multiple adapters.
            is_trainable (`bool`, *optional*, defaults to `False`):
                Whether the adapter should be trainable or not. If `False`, the adapter will be frozen and can only be
                used for inference.
        ...
        peft_model = None
        for adapter_name in adapter_names.keys():
            # load the config
            model_id = adapter_names[adapter_name]
            if config is None:
                config = PEFT_TYPE_TO_CONFIG_MAPPING[
                    PeftConfig._get_peft_type(
                        model_id,
    ...
    def merge_lora_weights(self, adapter_weights, adapters_weights, adapter_config):
        state_dict = adapter_weights
        peft_model_state_dict = adapters_weights
        parameter_prefix = {
            PeftType.LORA: "lora_",
        }[adapter_config.peft_type]
        for k, v in state_dict.items():
            if k in peft_model_state_dict.keys():
                k_weight_in_peft = peft_model_state_dict[k]
                # print("in merge", k,v.size(), k_weight_in_peft.size())
                if "A" in k: # lora_A
                    k_weight_in_peft = torch.cat([k_weight_in_peft, v], dim=0)
                else:# "lora_B"
                    k_weight_in_peft = torch.cat([k_weight_in_peft, v], dim=1)
                
            else:
                # print("new params")
                k_weight_in_peft = v
            peft_model_state_dict[k] = k_weight_in_peft    
            # if parameter_prefix in k:
            #     suffix = k.split(parameter_prefix)[1]
            #     prefix = k.split(parameter_prefix)[0]
            #     print(parameter_prefix, prefix, suffix)
            #     print(k, v.size())
    ...
    def load_adapter_only(self, model_id: str, adapter_name: str, is_trainable: bool = False, **kwargs: Any):
        """
        ...
        print("Load adapter,", adapter_name)
        ...
        adapter_weights = load_peft_weights(model_id, device=torch_device, **hf_hub_download_kwargs)
        return adapter_weights
    
    def load_weights_into_the_model(self, adapters_weights: Dict, adapter_name: str, is_trainable: bool = False, **kwargs: Any):
    ...
    def load_adapter(self, model_id: str, adapter_name: str, is_trainable: bool = False, **kwargs: Any):
        ...
        hf_hub_download_kwargs, kwargs = self._split_kwargs(kwargs)
        torch_device = infer_device()
        # print("in load adapter func, show base model arch", self)
...
class PeftModelForCausalLM(PeftModel):
        ...
        **kwargs,
        ...
        if not peft_config.is_prompt_learning:
            if self.base_model.config.model_type == "mpt":
                # print("in peftModelForCLM model_type is mpt")
        ...
        if peft_config.peft_type == PeftType.PREFIX_TUNING:
            past_key_values = self.get_prompt(batch_size)
            # print("in peftModelForCLM peft type is PREFIX_TUNING")
            return self.base_model(
                input_ids=input_ids, inputs_embeds=inputs_embeds, past_key_values=past_key_values, **kwargs
            )
        else:
            # print("in peftModelForCLM peft type is not PREFIX_TUNING")
            if inputs_embeds is None:
                inputs_embeds = self.word_embeddings(input_ids)
    ...
    def generate(self, **kwargs):
        ...
        try:
            # print("before peft model call base model's generate")
